...
import { productos, obtenerProducto, descontarExistencias } from "./sqlConexion";
...
export default function ModalFormulario({ visible, onClose, onGuardado }) {
  ...
  const handleGuardar = async () => {
    if (productoSeleccionado) {
      const result = await descontarExistencias(productoSeleccionado, formData.stock);
  ...
  return (
    <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[#58585a] text-white rounded-2xl w-full max-w-2xl shadow-lg relative max-h-[90vh] overflow-y-auto p-8 scrollbar-thin">
        <h2 className="text-2xl text-center mb-6">DESCONTAR EXISTENCIAS</h2>
        ...
        {/* precio */}
          <div>
            <label className="block uppercase text-sm mb-1">Indique la cantidad que desea descontar:</label>
            <input
              type="text"
              inputMode="numeric"
