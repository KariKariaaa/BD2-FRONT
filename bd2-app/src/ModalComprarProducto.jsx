...
import { productos, comprarProducto } from "./sqlConexion";
...
export default function ModalFormulario({ visible, onClose, onGuardado }) {
  ...
  const handleGuardar = async () => {
    if (productoSeleccionado) {
      const result = await comprarProducto(productoSeleccionado, formData.stock, localStorage.getItem("idUsuario"), localStorage.getItem('Rol'), formData.precio);
  ...
  return (
    <div className="fixed inset-0 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-[#58585a] text-white rounded-2xl w-full max-w-2xl shadow-lg relative max-h-[90vh] overflow-y-auto p-8 scrollbar-thin">
        <h2 className="text-2xl text-center mb-6">COMPRAR PRODUCTO</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            <label className="block uppercase text-sm mb-1">Cantidad a Comprar:</label>
            <input
              type="text"
              inputMode="numeric"
          ...
          {/* telefono */}
          <div>
            <label className="block uppercase text-sm mb-1">Precio:</label>
            <input
              type="text"
              inputMode="decimal"
